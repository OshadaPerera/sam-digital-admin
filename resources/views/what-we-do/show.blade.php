<x-layouts.app>
    <div x-data="albumShow">
                ...
                <div class="md:w-2/3">
                    <div class="flex items-start justify-between mb-4">
                        ...
                        <div class="flex items-center gap-2">
                            @can('edit what we do')
                                <a href="{{ route('what-we-do.edit', $album) }}" class="btn btn-outline-primary btn-sm">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                    @foreach ($album->images as $image)
                        <div class="relative group aspect-square rounded-lg overflow-hidden bg-gray-100 dark:bg-gray-700">
                            <img src="{{ asset('storage/' . $image->image_path) }}" alt="Album image" class="w-full h-full object-cover">
                            ...
                            <div class="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-center justify-center gap-2">
                                <button @click="viewImage('{{ asset('storage/' . $image->image_path) }}')" class="btn btn-sm bg-white/20 hover:bg-white/30 text-white border-0">
                                    <i class="fa-solid fa-eye"></i>
                                </button>
                                @can('delete what we do images')
                                    <button @click="deleteImage('{{ route('what-we-do.delete-image', [$album, $image]) }}')" class="btn btn-sm bg-danger/80 hover:bg-danger text-white border-0">
        ...
        {{-- Add Images Modal --}}
        <div x-show="showAddImagesModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto" @keydown.escape.window="showAddImagesModal = false">
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
                {{-- Backdrop --}}
                <div x-show="showAddImagesModal" @click="showAddImagesModal = false" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="fixed inset-0 bg-black/50 transition-opacity"></div>

                {{-- Modal Content --}}
                <div x-show="showAddImagesModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
                    ...
                    <form @submit.prevent="submitImages">
                        <div class="p-6">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="file" @change="handleImages" accept="image/*" multiple class="form-input">
                                ...
                                <div class="grid grid-cols-4 gap-3 mb-4">
                                    <template x-for="(preview, index) in imagePreviews" :key="index">
                                        <div class="relative aspect-square rounded-lg overflow-hidden bg-gray-100 dark:bg-gray-700">
                                            ...
                                            <button type="button" @click="removeImage(index)" class="absolute -top-1 -right-1 bg-danger text-white rounded-full h-5 w-5 flex items-center justify-center hover:bg-danger/80">
                        ...
                        <div class="bg-gray-50 dark:bg-gray-900 px-6 py-4 flex items-center justify-end gap-3">
                            <button type="button" @click="closeAddImagesModal" class="btn btn-outline-danger">
                                Cancel
                            </button>
                            <button type="submit" class="btn btn-primary" :disabled="submitting || imagePreviews.length === 0">
        ...
        {{-- View Image Modal --}}
        <div x-show="showViewImageModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto" @keydown.escape.window="showViewImageModal = false">
            <div class="flex items-center justify-center min-h-screen px-4">
                <div x-show="showViewImageModal" @click="showViewImageModal = false" class="fixed inset-0 bg-black/80 transition-opacity"></div>

                <div x-show="showViewImageModal" class="relative max-w-4xl w-full">
                    <button @click="showViewImageModal = false" class="absolute -top-10 right-0 text-white hover:text-gray-300">
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('albumShow', () => ({
                ...
                submitImages() {
                    ...
                    submitFormData('{{ route('what-we-do.add-images', $album) }}', formData).finally(() => {

<x-layouts.app>
    <div x-data="albumList">
        <ul class="flex space-x-2 rtl:space-x-reverse mb-6">
            <li>
                <a href="javascript:;" class="text-primary hover:underline font-semibold">What We Do</a>
            </li>
        </ul>

        <div class="panel">
            <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <h5 class="text-xl font-semibold dark:text-white-light">What We Do Albums</h5>
                ...
                <div class="flex items-center gap-3">
                    ...
                    @can('create what we do')
                        <a href="{{ route('what-we-do.create') }}" class="btn btn-primary">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
                    @foreach ($albums as $album)
                        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm overflow-hidden transition-all duration-200 hover:shadow-lg border border-gray-200 dark:border-gray-700">
                            {{-- Cover Image --}}
                            <div class="relative bg-gray-100 dark:bg-gray-700 w-full h-48">
                                @if ($album->cover_image)
                                    <img src="{{ asset('storage/' . $album->cover_image) }}" alt="{{ $album->title }}" class="w-full h-full object-contain" style="height: 12rem;">
                                @else
                                    <div class="w-full flex items-center justify-center" style="height: 12rem;">
                            ...
                            <div class="p-5">
                                ...
                                <div class="flex items-center gap-2 pt-4 border-t border-gray-200 dark:border-gray-700">
                                    @can('view what we do')
                                        <a href="{{ route('what-we-do.show', $album) }}" class="flex-1 btn btn-outline-primary btn-sm">
                                    ...
                                    @can('edit what we do')
                                        <a href="{{ route('what-we-do.edit', $album) }}" class="btn btn-outline-info btn-sm" x-tooltip.raw="Edit Album">
                                            <i class="fa-solid fa-pen"></i>
                                        </a>
                                    @endcan

                                    @if ($album->status === 'active')
                                        @can('deactivate what we do')
                                            <button type="button" class="btn btn-outline-warning btn-sm" x-tooltip.raw="Deactivate" @click="deactivate('{{ route('what-we-do.deactivate', $album->id) }}')">
                                                <i class="fa-solid fa-toggle-on"></i>
                                            </button>
                                        @endcan
                                    @else
                                        @can('activate what we do')
                                            <button type="button" class="btn btn-outline-success btn-sm" x-tooltip.raw="Activate" @click="activate('{{ route('what-we-do.activate', $album->id) }}')">
                                                <i class="fa-solid fa-toggle-off"></i>
                                            </button>
                                        @endcan
                                    @endif

                                    @can('delete what we do')
                                        <button type="button" class="btn btn-outline-danger btn-sm" x-tooltip.raw="Delete" @click="destroy('{{ route('what-we-do.destroy', $album->id) }}')">
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('albumList', () => ({
                ...
                filterAlbums() {
                    window.location.href = '{{ route('what-we-do.index') }}?status=' + this.status;
                },

    ...
    <div>
        ...
        <div class="panel mt-6">
            <div class="flex items-center justify-between mb-5">
                <h5 class="text-lg font-semibold dark:text-white-light">Edit Role: {{ $role->name }}</h5>
                <a href="{{ route('roles.index') }}" class="btn btn-outline-danger">
                    <i class="fa-solid fa-arrow-left mr-2"></i> Back
                </a>
            </div>

            <form action="{{ route('roles.update', $role) }}" method="POST" id="editRoleForm" x-data="roleForm">
                @method('PUT')
                @csrf
                ...
                <div class="space-y-5">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input id="role_name" type="text" name="name" placeholder="Enter role name" class="form-input" value="{{ old('name', $role->name) }}" />
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                        @foreach ($permissions as $permission)
                                            <label class="flex items-center cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700 p-3 rounded transition-colors">
                                                <input type="checkbox" name="permissions[]" value="{{ $permission }}" class="form-checkbox text-primary permission-checkbox" data-group="{{ Str::slug($group) }}" @change="updateGroupState('{{ Str::slug($group) }}')" {{ in_array($permission, $rolePermissions) ? 'checked' : '' }} />
                    ...
                    <div class="flex justify-end items-center gap-4 mt-8 pt-5 border-t border-gray-200 dark:border-gray-700">
                        ...
                        <button type="submit" @click.prevent="submitForm" class="btn btn-primary">
                            <i class="fa-solid fa-check mr-2"></i> Update Role
                        </button>
                    </div>
                </div>
            </form>
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('roleForm', () => ({
                ...
                submitForm() {
                    ...
                    submitFormData('{{ route('roles.update', $role) }}', formData);
                }
            }));
        });
    </script>
